...
The aim is to allow a _DSArray_ to be used as a drop-in replacement for an _array_ from the __base__ package when the need arises. The _DSArray_ API is therefore written to mimic the _array_ API so that _DSArray_ objects behave as if they were 3-dimensional _array_ objects.
